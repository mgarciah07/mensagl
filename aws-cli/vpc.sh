...
rtb_public_id=$(aws ec2 create-route-table --vpc-id "$vpc_id" --tag-specifications 'ResourceType=route-table,Tags=[{Key=Name,Value=vpc-mensagl-2025-Marcos-rtb-public}]' --query 'RouteTable.RouteTableId' --output text)
...
nat_id=$(aws ec2 create-nat-gateway --subnet-id "$subnet_public1_id" --allocation-id "$eipalloc_id" --tag-specifications 'ResourceType=natgateway,Tags=[{Key=Name,Value=vpc-mensagl-2025-Marcos-nat-public1-us-east-1a}]' --query 'NatGateway.NatGatewayId' --output text)

# Pausa para esperar que el NAT Gateway esté completamente operativo
sleep 10

# Crear tablas de rutas privadas
rtb_private1_id=$(aws ec2 create-route-table --vpc-id "$vpc_id" --tag-specifications 'ResourceType=route-table,Tags=[{Key=Name,Value=vpc-mensagl-2025-Marcos-rtb-private1-us-east-1a}]' --query 'RouteTable.RouteTableId' --output text)
...
rtb_private2_id=$(aws ec2 create-route-table --vpc-id "$vpc_id" --tag-specifications 'ResourceType=route-table,Tags=[{Key=Name,Value=vpc-mensagl-2025-Marcos-rtb-private2-us-east-1b}]' --query 'RouteTable.RouteTableId' --output text)
