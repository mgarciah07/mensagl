...
aws ec2 run-instances --image-id "$ami_idmatrix" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_matrix_synapse_id" --subnet-id "$subnet_public1_id" --associate-public-ip-address --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Matrix-Synapse1}]' --user-data file://../EC2-User/matrixdns.sh

# Crear instancias Wordpress en subredes publicas con IP pública
aws ec2 run-instances --image-id "$ami_idticket" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_wordpress_id" --subnet-id "$subnet_public2_id" --associate-public-ip-address --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Wordpress1}]' --user-data file://../EC2-User/ticketsdns.sh
